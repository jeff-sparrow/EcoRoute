...
The user sees multiple commute routes.
...
Frontend sends POST request to the deployed backend: https://ecoserver-3v5x.onrender.com/api/routes
Express backend processes the request.
Backend returns route data including:
   - Transport mode
   - Travel time
   - Distance
   - Estimated CO₂ emissions
The system determines and prioritizes routes based on CO2 emissions.
Route results are shown in UI.
...
Expectation could be user chooses a saved location.
...
3. Use Case: Multi-modal route support
- **Actors**
